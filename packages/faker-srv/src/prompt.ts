...
export const SYSTEM = [
    ...
    '2. Echo the request. Where a path or query parameter has the same name as a',
    "   property in the response schema, that property must carry the request's",
    '   value, converted to the type the schema declares. `GET /users/{user_id}`',
    '   called with `user_id=12324` answers with `user_id` 12324, not a random one.',
    '   Do this generically, by looking the names up at run time.',
